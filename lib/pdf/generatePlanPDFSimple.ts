...
export async function generatePlanPDFBuffer(planData: PlanData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      // Store footer function to call at the end
      const addFooter = () => {
        // US Letter page height is 792 points, with 40pt bottom margin
        // Position footer absolutely at the bottom
        const pageHeight = 792; // US Letter height in points
        const bottomMargin = 40;
        const footerY = pageHeight - bottomMargin - 12; // 12pt padding from bottom margin
        
        doc.fontSize(7).font('Times-Roman').fillColor('#9ca3af');
        doc.text('Educational estimate — not financial advice. Numbers may vary based on withholding, benefits, and timing.', 
          50, footerY, { 
            align: 'center', 
            width: pageWidth 
          });
        doc.text('Generated by WeLeap • weleap.ai', 
          50, footerY + 10, { 
            align: 'center', 
            width: pageWidth 
          });
      ...
      // ========== FOOTER ==========
      // Add footer at absolute bottom position before ending document
      addFooter();
