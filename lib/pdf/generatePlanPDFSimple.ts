    ...
    try {
      ...
      try {
        ...
        if (fs.existsSync(logoPath)) {
          doc.image(logoPath, 50, 48, { width: 40, height: 40 });
        }
      } catch (logoError) {
      ...
      // CTA row
      doc.rect(50, yPos, 160, 24).fillAndStroke('#3F6B42', '#3F6B42');
      doc.fontSize(9).font('Times-Bold').fillColor('#ffffff').text('Join the WeLeap Beta', 55, yPos + 7);
      
      doc.fontSize(8).font('Times-Roman').fillColor('#3F6B42').text('Learn how the Sidekick works', 220, yPos + 8, { link: 'https://www.weleap.ai' });
      yPos += 35;
