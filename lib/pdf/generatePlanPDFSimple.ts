...
export async function generatePlanPDFBuffer(planData: PlanData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      // ========== HEADER ==========
      // Left: WeLeap logo (text for now)
      // Center: Title
      // Right: Meta
      doc.fontSize(10).font('Times-Roman').fillColor('#6b7280').text('WeLeap', 50, 50);
      doc.fontSize(22).font('Times-Bold').fillColor('#111827').text('Your Day 1 Playbook', { align: 'center', y: 48 });
      doc.fontSize(9).font('Times-Roman').fillColor('#6b7280').text('Built from your salary, rent, and start date', { align: 'center', y: 68 });
      ...
      doc.fontSize(16).font('Times-Bold').fillColor('#111827').text('Your Real Monthly Take-Home', 50, yPos);
      yPos += 25;
      
      // BIG NUMBER (largest on page)
      doc.fontSize(36).font('Times-Bold').fillColor('#111827').text(formatCurrency(planData.takeHomeMonthly), 50, yPos);
      yPos += 45;
      
      // Supporting line
      doc.fontSize(10).font('Times-Roman').fillColor('#6b7280').text('That\'s what actually lands in your account after taxes.', 50, yPos);
      yPos += 25;
      ...
      if (planData.taxBreakdown) {
        ...
        doc.fontSize(8).font('Times-Roman').fillColor('#6b7280').text('How we estimated taxes:', 50, yPos);
        yPos += 12;
        doc.fontSize(7).font('Times-Roman').fillColor('#6b7280').text(`Federal: -${formatCurrency(planData.taxBreakdown.federalTaxAnnual)}`, 60, yPos);
        doc.fontSize(7).font('Times-Roman').fillColor('#6b7280').text(`State: -${formatCurrency(planData.taxBreakdown.stateTaxAnnual)}`, 60 + 130, yPos);
        doc.fontSize(7).font('Times-Roman').fillColor('#6b7280').text(`FICA: -${formatCurrency(planData.taxBreakdown.ficaTaxAnnual)}`, 60 + 260, yPos);
        yPos += 15;
        doc.fontSize(7).font('Times-Italic').fillColor('#9ca3af').text('Estimate only — real withholding varies.', 60, yPos);
        yPos += 25;
      } else {
      ...
      doc.fontSize(16).font('Times-Bold').fillColor('#111827').text('Rent Reality Check', 50, yPos);
      yPos += 25;
      ...
      const highlightBoxWidth = 200;
      const highlightBoxHeight = 70;
      ...
      yPos += highlightBoxHeight + 10;
      doc.fontSize(9).font('Times-Roman').fillColor('#6b7280').text('If your rent is above this range, you\'ll likely feel tighter on savings and flexibility.', 50, yPos);
      yPos += 30;
      ...
      if (planData.upfrontCashLow && planData.upfrontCashHigh) {
        ...
        doc.fontSize(16).font('Times-Bold').fillColor('#111827').text('Cash Needed Before Your First Paycheck', 55, panelY + 10);
        
        // BIG NUMBER
        doc.fontSize(32).font('Times-Bold').fillColor('#111827').text(`${formatCurrency(planData.upfrontCashLow)}–${formatCurrency(planData.upfrontCashHigh)}`, 55, panelY + 35);
        
        // Subhead
        doc.fontSize(9).font('Times-Roman').fillColor('#6b7280').text('This is why the first few weeks can feel tight — even with a good salary.', 55, panelY + 70);
        
        // 2-column breakdown
        const col1X = 55;
        const col2X = 55 + (pageWidth / 2);
        const breakdownY = panelY + 85;
        ...
        // Quick Tip
        doc.fontSize(7).font('Times-Italic').fillColor('#6b7280').text('Tip: Plan this as "startup cash," not a monthly expense.', 55, panelY + panelHeight - 15);
      ...
      if (planData.budgetBreakdown) {
        doc.fontSize(16).font('Times-Bold').fillColor('#111827').text('A Simple Starting Structure (First 90 Days)', 50, yPos);
        yPos += 25;
        ...
        // Needs (50%)
        doc.fontSize(11).font('Times-Bold').fillColor('#111827').text('Needs (baseline):', 50, yPos);
        doc.fontSize(10).font('Times-Roman').fillColor('#6b7280').text('50%', 50, yPos + 15);
        doc.rect(50, yPos + 25, structBarWidth, 15).fill('#ef4444');
        doc.fontSize(12).font('Times-Bold').fillColor('#111827').text(`→ ${formatCurrency(planData.budgetBreakdown.needs)}`, 50, yPos + 45);
        
        // Wants (30%)
        doc.fontSize(11).font('Times-Bold').fillColor('#111827').text('Wants (baseline):', 50 + structColWidth, yPos);
        doc.fontSize(10).font('Times-Roman').fillColor('#6b7280').text('30%', 50 + structColWidth, yPos + 15);
        doc.rect(50 + structColWidth, yPos + 25, structBarWidth * 0.6, 15).fill('#f59e0b');
        doc.fontSize(12).font('Times-Bold').fillColor('#111827').text(`→ ${formatCurrency(planData.budgetBreakdown.wants)}`, 50 + structColWidth, yPos + 45);
        
        // Savings (20%)
        doc.fontSize(11).font('Times-Bold').fillColor('#111827').text('Savings (baseline):', 50 + structColWidth * 2, yPos);
        doc.fontSize(10).font('Times-Roman').fillColor('#6b7280').text('20%', 50 + structColWidth * 2, yPos + 15);
        doc.rect(50 + structColWidth * 2, yPos + 25, structBarWidth * 0.4, 15).fill('#10b981');
        doc.fontSize(12).font('Times-Bold').fillColor('#111827').text(`→ ${formatCurrency(planData.budgetBreakdown.savings)}`, 50 + structColWidth * 2, yPos + 45);
        
        yPos += 70;
        doc.fontSize(8).font('Times-Roman').fillColor('#6b7280').text('This isn\'t a rule — it\'s a baseline to keep you steady while everything is new.', 50, yPos);
        doc.fontSize(7).font('Times-Italic').fillColor('#9ca3af').text('Small, consistent moves beat perfect planning.', 50, yPos + 12);
        yPos += 30;
      }
      
      // ========== SECTION E: YOUR CONTEXT (QUIET) ==========
      doc.fontSize(12).font('Times-Bold').fillColor('#6b7280').text('Your Details', 50, yPos);
      yPos += 15;
      
      const formattedStartDate = formatDate(planData.startDate);
      doc.fontSize(9).font('Times-Roman').fillColor('#6b7280').text(`City: ${planData.city}`, 50, yPos);
      if (formattedStartDate) {
        doc.fontSize(9).font('Times-Roman').fillColor('#6b7280').text(`Start date: ${formattedStartDate}`, 50, yPos + 12);
      }
      doc.fontSize(9).font('Times-Roman').fillColor('#6b7280').text(`Days until start: ${planData.daysUntilStart} ${planData.daysUntilStart === 1 ? 'day' : 'days'}`, 50, yPos + 24);
      yPos += 45;
      
      // ========== SECTION F: WELEAP HANDOFF ==========
      doc.fontSize(16).font('Times-Bold').fillColor('#111827').text('What Happens Next (If You Use WeLeap)', 50, yPos);
      yPos += 25;
      
      doc.fontSize(10).font('Times-Roman').fillColor('#374151').text('This playbook is a snapshot. WeLeap turns it into a live feed that helps you stay on track as real life hits.', 50, yPos, { width: pageWidth });
      yPos += 25;
      
      // Bullets
      doc.fontSize(9).font('Times-Roman').fillColor('#374151').text('• Detects key moments (paychecks, rent payments)', 55, yPos);
      yPos += 15;
      doc.fontSize(9).font('Times-Roman').fillColor('#374151').text('• Flags risks early (low balance, spending spikes)', 55, yPos);
      yPos += 15;
      doc.fontSize(9).font('Times-Roman').fillColor('#374151').text('• Surfaces one clear next move at a time ("Leaps")', 55, yPos);
      yPos += 30;
      
      // CTA row
      doc.rect(50, yPos, 180, 28).fillAndStroke('#3F6B42', '#3F6B42');
      doc.fontSize(10).font('Times-Bold').fillColor('#ffffff').text('Join the WeLeap Beta', 55, yPos + 8);
      
      doc.fontSize(9).font('Times-Roman').fillColor('#3F6B42').text('Learn how the Sidekick works', 250, yPos + 9, { link: 'https://www.weleap.ai' });
      yPos += 50;
      
      // ========== FOOTER ==========
      const footerY = doc.page.height - 60;
      doc.fontSize(7).font('Times-Roman').fillColor('#9ca3af').text('Educational estimate — not financial advice. Numbers may vary based on withholding, benefits, and timing.', { align: 'center', y: footerY });
      doc.fontSize(7).font('Times-Roman').fillColor('#9ca3af').text('Generated by WeLeap • weleap.ai', { align: 'center', y: footerY + 10 });
